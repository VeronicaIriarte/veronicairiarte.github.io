...
function formatPrice(price) {
  if (price === null || price === undefined || price === '') return 'Valor no disponible';
  if (typeof price === 'number') {
    return 'USD\u00A0' + new Intl.NumberFormat('es-AR').format(price);
  }
  if (typeof price === 'string') {
    const normalized = price.trim();
    if (!normalized) return 'Valor no disponible';
    if (normalized === 'no_vende') return 'No disponible para la venta';
    if (normalized === 'vendido') return 'Vendido · colección privada';
    if (normalized === 'vendido_noruega') return 'Vendido · colección privada, Noruega';
    return normalized;
  }
  return String(price);
}
...
function setCategory(category) {
  ...
  if (category === 'comprar') {
    ...
    if (intro) {
      ...
      categoryIntroText.replaceChildren();
      intro.paragraphs.forEach((paragraph) => {
        const p = document.createElement('p');
        p.textContent = paragraph;
        categoryIntroText.appendChild(p);
      });
